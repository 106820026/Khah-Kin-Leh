    ...
    if(color_code_strings.length > 0) {
        if(colored_strings.length == 0) {
                ...
                for(str_dom of color_code_strings) {
                    for(element of color_codes){
                        if(str_dom.innerHTML.includes(element)) {
                            str_dom.innerHTML = str_dom.innerHTML.replaceAll(element, "<span class=\"ivy_add_color_code\" style=\"background-color: " + color_dict[element] + "; color: black\">" + element + "</span>")
                        }
                    }
                }
            })
            ...
            for(str_dom of color_code_strings) {
                for(element of color_codes){
                    if(str_dom.innerHTML.includes(element)) {
                        str_dom.innerHTML = str_dom.innerHTML.replaceAll("<span class=\"ivy_add_color_code\" style=\"background-color: " + color_dict[element] + "; color: black\">" + element + "</span>", element)
                    }
                }
            }
        }
